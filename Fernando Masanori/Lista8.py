...
def maior_ponta(nums):
   ...
   ultimo = nums[-1]
   if nums[0] > nums[-1]:
     return [primeiro] * len(nums)
   else:
     return  [ultimo] * len(nums)
...
def pego_correndo(speed, is_birthday):
    # Se for aniversário, podemos aumentar 5 km/h no limite
    if is_birthday:
        speed -= 5  # subtraímos 5 para aplicar os limites normais

    # Agora aplicamos as faixas de velocidade
    if speed <= 60:
        return 0  # sem multa
    elif speed <= 80:
